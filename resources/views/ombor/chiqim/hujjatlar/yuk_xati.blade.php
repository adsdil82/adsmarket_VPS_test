...
<h2>YUK XATI</h2><p class='sub'>№ CHQ-{{ $chiqim->id }}-YX &nbsp;|&nbsp; {{ $chiqim->sana->format('d.m.Y') }}</p>
<table class='info'>
<tr><td>Beruvchi tashkilot</td><td>{{ $chiqim->filial?->nomi }}</td></tr>
<tr><td>Sabab</td><td>{{ \App\Models\OmbordanChiqim::$sabablar[$chiqim->sabab] ?? $chiqim->sabab }}</td></tr>
...
<p>Yuqorida ko'rsatilgan tovarlar omborxonadan chiqarildi.</p>
<div class='imzo'><table><tr>
<td width='50%'>Berdi (omborchi): ___________________<br>{{ $chiqim->filial?->nomi }}</td>
<td width='50%'>Qabul qildi: ___________________<br>Sana: ___________________</td>
