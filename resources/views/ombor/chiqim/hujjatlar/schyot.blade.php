...
<h2>SCHYOT-FAKTURA (ICHKI HISOBOT)</h2><p class='sub'>№ CHQ-{{ $chiqim->id }}-SF &nbsp;|&nbsp; {{ $chiqim->sana->format('d.m.Y') }}</p>
<table class='info'>
<tr><td>Tashkilot</td><td>{{ $chiqim->filial?->nomi }}</td></tr>
<tr><td>Hujjat turi</td><td>Ombordan chiqim — {{ \App\Models\OmbordanChiqim::$sabablar[$chiqim->sabab] ?? $chiqim->sabab }}</td></tr>
<tr><td>Mas'ul xodim</td><td>{{ $chiqim->xodim?->ism_familiya }}</td></tr>
</table>
...
<tr><td align='center'>{{ $i+1 }}</td><td>{{ $t->tovar?->nomi }}</td><td align='center'>{{ $t->tovar?->birlik ?? 'dona' }}</td>
<td align='center'>{{ $t->miqdor }}</td><td align='right'>{{ number_format($t->narx,0,'.',' ') }}</td>
<td align='right'>{{ number_format($t->jami_summa,0,'.',' ') }}</td></tr>
...
<td width='50%'>Tuzdi: ___________________<br>{{ $chiqim->xodim?->ism_familiya }}</td>
