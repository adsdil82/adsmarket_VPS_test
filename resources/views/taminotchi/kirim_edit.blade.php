...
<div class="bft-header-card mb-3">
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <span class="fw-bold">{{ $taminotchi->nomi }} — Kirimni tahrirlash</span>
        <span class="badge bg-light text-dark">{{ $kirim->kirim_sana->format('d.m.Y') }}</span>
    </div>
...
<div class="bft-wrap mb-3">
    <div class="bft-inforow">
        ...
        <div class="bft-cell-value">
            <input type="text" name="hujjat_raqam" class="form-control form-control-sm" style="width:180px" value="{{ old('hujjat_raqam', $kirim->hujjat_raqam) }}" placeholder="SF-2025-001">
        </div>
        <div class="bft-cell-label">Kirim sanasi <span class="text-danger">*</span></div>
        <div class="bft-cell-value">
            <input type="date" name="kirim_sana" class="form-control form-control-sm" style="width:150px" value="{{ old('kirim_sana', $kirim->kirim_sana->toDateString()) }}" required>
        </div>
    ...
    <div class="bft-inforow">
        ...
        <div class="bft-cell-value bft-grow">
            <textarea name="izoh" class="form-control form-control-sm auto-expand" style="width:100%" rows="1" placeholder="Ixtiyoriy...">{{ old('izoh', $kirim->izoh) }}</textarea>
        </div>
    </div>
</div>
...
MAVJUD_QATORLAR.forEach(function(q) {
    ...
    if (q.guruh_id) {
        ...
        guruhOzgardi(tr); // faqat datalistni to'ldirish uchun — nomi/tovar_id pastda qayta belgilanadi
    }
    ...
    // Ustama ustunlarini har doim ko'rsatamiz — mavjud tovar bo'lsa, katalogdagi
    // joriy POS/Nasiya narxlaridan foiz hisoblab oldindan to'ldiramiz (tahrirlash uchun).
    ustamaUstunlarKorsat(tr, true);
    var mos = q.tovar_id ? BARCHA_TOVARLAR.find(function(t) { return String(t.id) === String(q.tovar_id); }) : null;
    if (mos) {
        tr.querySelector('.yangi-tovar-belgi').style.display = 'none';
        katalogNarxlariniOldindanTuldirish(tr, mos);
    } else {
        tr.querySelector('.yangi-tovar-belgi').style.display = q.guruh_id ? 'inline-flex' : 'none';
        ustamaHisobla(tr, 'foiz');
    }
});
