@extends('layouts.app')
@section('title', 'Kreditga sotilgan tovarlar')
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('hisobotlar.index') }}">Hisobotlar</a></li>
    <li class="breadcrumb-item active">Kreditga sotilgan tovarlar</li>
@endsection
...
/* ── Bank uslubidagi jadval — skromniy, yengil ranglar ── */
...
.bank-table tbody tr:nth-child(even) td { background:#f3faf5; }
...
.bft-farqi-manfiy { color:#dc2626 !important; font-weight:700; }
.bft-farqi-musbat { color:#15803d !important; font-weight:700; }
</style>
@endpush
...
<div class="bft-header-card mb-3">
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <span class="fw-bold">Kreditga sotilgan tovarlar</span>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('hisobotlar.excel','sotilgan_tovarlar') }}?dan_sana={{ $danSana }}&gacha_sana={{ $gachaSana }}&filial_id={{ $filialId }}"
...
<div class="bft-stat-wrap mb-3">
    ...
    <div class="bft-stat">
        <div class="lbl">Kirim (tan narx)</div>
        <div class="val" style="color:#64748b">{{ number_format($jami->kirim_summasi/1000000,1) }} mln</div>
    </div>
    <div class="bft-stat">
        <div class="lbl">Sotilgan summa</div>
        <div class="val" style="color:#16a34a">{{ number_format($jami->sotilgan_summasi/1000000,1) }} mln</div>
    </div>
    <div class="bft-stat">
        <div class="lbl">Ustama ({{ $jami->ustama_foizi }}%)</div>
        <div class="val" style="color:#d97706">{{ number_format($jami->farqi/1000000,1) }} mln</div>
    </div>
</div>
    ...
    <table class="bank-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="num">{{ number_format($t->kirim_summasi,0,'.',' ') }}</td>
                <td class="num">{{ number_format($t->sotilgan_summasi,0,'.',' ') }}</td>
                <td class="num {{ $t->farqi >= 0 ? 'bft-farqi-musbat' : 'bft-farqi-manfiy' }}">
                    {{ number_format($t->farqi,0,'.',' ') }}
                </td>
                <td class="num">
                    <span class="badge bg-{{ $t->ustama_foizi >= 20 ? 'primary' : ($t->ustama_foizi >= 0 ? 'warning' : 'danger') }}" style="font-size:.7rem">
                        {{ $t->ustama_foizi }}%
                    </span>
                </td>
            </tr>
            @empty
            <tr><td colspan="9" class="text-center py-4">Tanlangan davrda sotilgan tovar topilmadi</td></tr>
            @endforelse
        </tbody>
        ...
        <tfoot>
            <tr>
                ...
                <td class="num">—</td>
                <td class="num">{{ number_format($jami->kirim_summasi,0,'.',' ') }}</td>
                <td class="num">{{ number_format($jami->sotilgan_summasi,0,'.',' ') }}</td>
                <td class="num">{{ number_format($jami->farqi,0,'.',' ') }}</td>
                <td class="num">{{ $jami->ustama_foizi }}%</td>
            </tr>
        </tfoot>
        @endif
    </table>
